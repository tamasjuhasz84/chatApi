...
io.on('connection', socket => {
    ...
    logger.info(`${cookies.username} nevű felhasználó ${socket.id} socketen kapcsolódott a beszélgetéshez!`)

    // ez meg mindenki másnak 
    socket.broadcast.emit('commonMessage', cookies.username + " kapcsolódott a beszélgetéshez")
    ...
    socket.on('disconnect', () => {
        socket.broadcast.emit('commonMessage', cookies.username + " kilépett a beszélgetésből!")
    })
...
app.get('/', (req, res) => {
  ...
  app.post('/process_login', (req, res) => {
    ...
    (async () => {
      const selectedPass = await sequelize.query("SELECT pw FROM `registredUsers` WHERE name LIKE :search_name", {
        replacements: { search_name: username }, type: QueryTypes.SELECT
      });
      if (!selectedPass[0]) {
        res.cookie('loginError', "Hibás jelszó vagy felhasználónév!")
        res.redirect('/')
        logger.warn(`Jogosulatlan belépési kísérlet a következő adatokkal: felhasználónév: ${username}, jelszó: ${password}`)
      } else {
  ...
  app.post('/process_create', (req, res) => {
    ...
    (async () => {
        const selectedUserName = await sequelize.query("SELECT name FROM `registredUsers` WHERE name LIKE :search_name", {
          replacements: { search_name: username }, type: QueryTypes.SELECT
        });
